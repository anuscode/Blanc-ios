...
@objc public protocol SlideDelegate {
    func didFinishSlideToOpenDelegate(_ sender: Slide)
}

@objcMembers public class Slide: UIView {
    // MARK: All Views
    ...
    lazy public var sliderTextLabel: UILabel = {
        let label = UILabel()
        return label
    }()

    lazy public var thumbnailImageView: UIImageView = {
    ...
    lazy public var sliderHolderView: UIView = {
        let view = UIView()
        view.addSubview(holderShimmer)
        return view
    }()
    ...
    lazy public var draggedView: UIView = {
        let view = UIView()
        return view
    }()
    ...
    public weak var delegate: SlideDelegate?
    ...
    public var textFont: UIFont = .systemFont(ofSize: 18.0) {
        didSet {
    ...
    public var isShimmering: Bool = true {
        didSet {
            holderShimmer.contentView = sliderHolderView
            holderShimmer.isShimmering = isShimmering
        }
    }
    ...
    private func setupConstraint() {
        ...
        // Setup for slider holder view
        holderShimmer.snp.makeConstraints { make in
            make.edges.equalTo(sliderHolderView.snp.edges)
        }
    ...
    @objc private func handlePanGesture(_ sender: UIPanGestureRecognizer) {
        ...
        case .ended:
            if translatedPoint >= xEndingPoint {
                ...
                // Finish action
                isFinished = true
                delegate?.didFinishSlideToOpenDelegate(self)
                return
            }
            if translatedPoint <= thumbnailViewStartingDistance {
                textLabel.alpha = 1
