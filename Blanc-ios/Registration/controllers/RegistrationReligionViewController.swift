...
class RegistrationReligionViewController: UIViewController {
    ...
    lazy private var progressView: UIProgressView = {
        ...
        progress.progress = 8 / RConfig.progressCount
        return progress
    }()
    ...
    private func update() {
        if (user?.religionId == nil) {
            return
        }

        let indexPath = IndexPath(row: user!.religionId!, section: 0)
    ...
    private func presentNextView() {
        ...
        navigation.present(identifier: "RegistrationDrinkViewController")
    }

    private func presentBackView() {
        let navigation = navigationController as! RegistrationNavigationViewController
        navigation.present(identifier: "RegistrationEducationViewController", animated: false)
    }
}


extension RegistrationReligionViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == user?.religionId {
            cell.select()
        }
        return cell
    }
